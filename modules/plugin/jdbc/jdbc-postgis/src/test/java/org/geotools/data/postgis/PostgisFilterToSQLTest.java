...
public class PostgisFilterToSQLTest extends SQLFilterTestSupport {
    ...
    @Test
    public void testEncodePgAnyCapabilities() throws Exception {
        filterToSql.setFeatureType(testSchema);
        PropertyIsEqualTo expr =
                ff.equals(ff.function("pgAny", ff.property("testArray")), ff.literal(5));
    ...
    @Test
    public void testEncodePgAny() throws Exception {
        PropertyIsEqualTo expr =
                ff.equals(ff.function("pgAny", ff.property("testArray")), ff.literal(5));

        filterToSql.encode(expr);
        String sql = writer.toString().toLowerCase();
        assertEquals("where any(testarray) = 5", sql);
    }
}
