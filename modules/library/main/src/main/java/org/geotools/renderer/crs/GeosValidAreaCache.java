 ...
 *
 * <p>Compute is expensive; cache is SoftValueHashMap-backed (thread-safe) so values may be
 * reclaimed under memory pressure.
 *
 * <p>The valid-area is computed *from the GEOS CRS itself* by probing the exact GeoTools forward
 * transform domain (works for the Ellipsoidal implementation too).
 */
final class GeosValidAreaCache {
    ...
    /** GEOS CRS -> valid area. */
    private final SoftValueHashMap<CoordinateReferenceSystem, Geometry> validAreaCache =
    ...
    /**
     * Builds the valid area geometry in WGS84 by probing the forward transform domain of the GEOS
     * CRS.
     *
     * @param geostationary the GEOS CRS to probe and extract parameters from
     * @return a Geometry in WGS84 representing the valid area of the GEOS CRS
     * @throws FactoryException if there is an error accessing the CRS parameters or creating the
     *     math transform
     */
    private static Geometry buildValidAreaWgs84(CoordinateReferenceSystem geostationary)
            throws FactoryException {
        // Extract what we need from the CRS itself
        if (!(geostationary instanceof ProjectedCRS)) {
            throw new IllegalArgumentException(
                    "Expected a ProjectedCRS, got: " + geostationary.getClass());
        }
        ...
        // Compute a conservative "angular radius" to compute the max visible central angle, and
        ...
        return buildValidAreaWgs84(
                geostationary, centralMeridian, angle, ellipsoid, AZIMUTH_SAMPLES, BISECTION_ITERS);
    }

    /**
     * Angular radius of the visible disk, as a bound for probing.
     *
     * <p>We just need an upper bound for binary search; correctness comes from probing. For
     * geostationary height H above surface and Earth radius R: central angle to limb ~
     * acos(R/(R+H)).
     *
     * <p>For ellipsoid, use semi-major axis as a conservative bound.
     */
    private static double computeAngularRadiusDeg(
            Ellipsoid ellipsoid, double satelliteHeightMeters) {
    ...
    /**
     * Compute the destination point from a start point, azimuth and distance using the geodetic
     * calculator.
     */
    private static Point2D destination(
            GeodeticCalculator gc, double lon0, double azDeg, double distMeters) {
    ...
    private static Geometry normalizeDateline(Geometry poly) {
        ...
        // update coordinates to be in the -180,180 range for easier intersection; we will translate
        // back the pieces
        // later
