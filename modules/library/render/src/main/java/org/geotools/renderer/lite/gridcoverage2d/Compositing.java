...
/**
 * A compositing element used in rendering. It may specify a Simple {@link BlendComposite} instance to define the type of compositing to be applied to
 * the coverage holding this property. It may also specify a compositing image to be drawn on the current rendering as part of the compositing
 * process.
 * 
...
class Compositing {
    ...
    private static Composite DEFAULT_COMPOSITE = BlendComposite.getInstance(BlendingMode.MULTIPLY, 1.0f);
    ...
    public GridCoverage2D composeGridCoverage(GridCoverage2D source, GridCoverageFactory factory) {
        if (compositingImage != null) {
            ...
            // in memory
            BufferedImage composedImage = PlanarImage.wrapRenderedImage(sourceImage)
                    .getAsBufferedImage();
            ...
            if (minX != 0 || minY != 0) {
                // I'm going to draw a RenderedImage on the BufferedImage source which has (0,0) origin.
            ...
            return factory.create(source.getName().toString(), gridCoverageImage,
                    (GridGeometry2D) source.getGridGeometry(), sd, new GridCoverage[] { source },
        ...
        if (finalImage != null) {
            ...
            if (!(cm instanceof ComponentColorModel) || numComponents < 3) {
                if (cm instanceof IndexColorModel) {
                    ...
                    iw = iw.addBands(new RenderedImage[] { finalImage, finalImage, finalImage },
                            false, null);
                }
