...
/**
 * This {@link CoverageProcessingNode} is responsible for visiting the supplied {@link ColorMapTransform} and applying it to the source
 * {@link GridCoverage2D} .
 * <p>
 * <strong>What we support and how do we implement it</strong>
 * <p>
 * A ColorMapTransform is created in order to map categories to colors on a single band coverage (or on the visible band of multiband coverage).
 * <p>
 * In this implementation we allow users to use either 256 or 65536 colors via the creation of a paletted image with s suitable palette derived from
 * the single {@link ColorMapEntry} that make up the {@link ColorMapTransform} .
 * 
 * @author Simone Giannecchini, GeoSolutions
 * @see ColorMapTransform
 */
class ColorMapNode extends StyleVisitorCoverageProcessingNodeAdapter implements StyleVisitor,
     ...
     * Possible types are
     * <ol>
     * <li>TYPE_RAMP which will build a ramp f colors using linear interpolation</li>
     * <li>TYPE_INTERVALS which would do classifications, i.e. would map intervals to colors</li>
    ...
    /**
     * The {@link Domain1D} that we build while parsing the various {@link ColorMapEntry}s provided by the {@link ColorMapTransform} we visit. This
     * {@link LinearColorMap} we'll help us buld the colormapped image.
     */
    ...
    /**
     * Visits the provided {@link ColorMapTransform} and build up a {@link Domain1D} for later creation of a palette rendering for this coverage.
     */
    public void visit(ColorMap colorMap) {
        ...
        if (cmEntries != null && cmEntries.length > 0) {
            ...
            builder.setExtendedColors(this.extendedColors).setLinearColorMapType(this.type)
    ...
    public ColorMapNode(Hints hints) {
        super(1, hints, SimpleInternationalString.wrap("ColorMapNode"), SimpleInternationalString
                .wrap("Node which applies a ColorMapTransform following SLD 1.0 spec."));
    }

    /**
     * Note that the color map can be applied only to a single band hence, in principle, applying the {@link ColorMapTransform} element to a coverage
     * with more than one band is an error.
     */
    protected GridCoverage2D execute() {
        ...
        if (colorMapTransform != null) {
            ...
            for (int i = 0; i < numBands; i++)
                sd[i] = new GridSampleDimension(TypeMap.getColorInterpretation(
                        classified.getColorModel(), i).name());
            ...
            return getCoverageFactory().create(
                    "color_mapped_" + sourceCoverage.getName().toString(), classified,
                    sourceCoverage.getGridGeometry(), sd, new GridCoverage[] { sourceCoverage },
                    properties);
        }
