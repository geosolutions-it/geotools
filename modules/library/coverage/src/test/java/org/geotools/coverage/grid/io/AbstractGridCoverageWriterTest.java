...
/**
 * Verifies that {@link AbstractGridCoverageWriter#getImageOutputStream} picks the right stream type
 * based on image size and destination type. Uses {@link ConstantDescriptor} images which are lazy
 * (no pixel RAM allocated) so large dimensions don't cause OOM.
 */
public class AbstractGridCoverageWriterTest {
    ...
    public void testOutputStreamSmallImageUsesMemoryCache() throws IOException {
        ...
        try {
            assertTrue(
                    writer.openStream(constantImage(100, 100))
                            instanceof MemoryCacheImageOutputStream);
        } finally {
            writer.dispose();
        }
    }
    ...
    public void testOutputStreamLargeImageUsesFileCache() throws IOException {
        // 100000x100000x1 byte = 10^10 bytes >> threshold=1 → file cache (no OOM:
        // ConstantDescriptor is lazy)
        ImageIOExt.setFilesystemThreshold(1L);
        TestWriter writer = new TestWriter(new ByteArrayOutputStream());
        try {
            assertTrue(
                    writer.openStream(constantImage(100_000, 100_000))
                            instanceof FileCacheImageOutputStream);
        } finally {
            writer.dispose();
        }
    }
