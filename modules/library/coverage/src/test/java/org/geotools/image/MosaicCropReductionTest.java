...
public class MosaicCropReductionTest {
    ...
    /**
     * The inner cropping operation is a single source Mosaic, which is what {@link
     * org.geotools.coverage.processing.operation.Crop} builds when nodata is involved.
     */
    @Test
    ...
    /**
     * A ROI on the Crop does not block the reduction: the operation reduces the ROI to its bounding
     * box, so it can only shrink the crop bounds, never mask pixels inside them, and the bounds are
     * checked already. A triangle whose bounds cover the crop therefore masks nothing at all.
     */
    @Test
    ...
    /**
     * The fusion works because the crop advertises its nodata range as a GC_NODATA property, so the
     * outer mosaic was already skipping the fill. Checked with a range wider than the fill, and
     * hole pixels that differ from it, so the two would part ways if that stopped holding.
     */
    @Test
    ...
    /**
     * The outer mosaic carrying thresholds must still honour the nodata range the fusion moves into
     * it: thresholds only stand in for a nodata that is not there.
     */
    @Test
    ...
    private RenderedImage buildMosaicOverCrop(
            boolean reductionEnabled, Rectangle layoutBounds, ROI cropRoi) {
    ...
    private RenderedImage buildMosaicOverMosaic(boolean reductionEnabled, ROI innerRoi) {
        ImageWorker.MOSAIC_CROP_REDUCTION_ENABLED = reductionEnabled;
        RenderedImage inner =
                singleSourceMosaic(source(0, 1), FULL, new double[] {FILL}, innerRoi, NODATA, null);
    ...
    private RenderedImage singleSourceMosaic(
        ...
        iw.setRenderingHint(JAI.KEY_IMAGE_LAYOUT, layout(layoutBounds));
    ...
    private static Matcher<RenderedImage> isOperation(String operation) {
        return new TypeSafeMatcher<RenderedImage>() {

            @Override
            protected boolean matchesSafely(RenderedImage image) {
                return image instanceof RenderedOp
                        && operation.equals(((RenderedOp) image).getOperationName());
            }
            ...
            protected void describeMismatchSafely(RenderedImage image, Description mismatch) {
                mismatch.appendText(
                        image instanceof RenderedOp
                                ? "the " + ((RenderedOp) image).getOperationName() + " operation"
    ...
    /**
     * Three band byte image with a uniform {@code holeValue} block standing in for nodata. Every
     * other sample is kept at or above {@code validFloor}, so only the block matches the nodata
     * range under test.
     */
    private RenderedImage source(int holeValue, int validFloor) {
        ...
        for (int y = 0; y < SRC_H; y++) {
            for (int x = 0; x < SRC_W; x++) {
                ...
                for (int b = 0; b < 3; b++) {
                    int sample =
                            nodata ? holeValue : validFloor + (v + 30 * b) % (256 - validFloor);
