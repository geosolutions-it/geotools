...
public class CoverageProcessor {
    ...
    /**
     * Removes the specified operation from this processor. The removal is atomic, but it is not
     * ordered against the put/rollback pair in {@link #addOperation(Operation)}: callers that may
     * run concurrently with an operation being added have to serialize themselves, the way {@link
     * #removeOperationFromProcessors(String)} does by being static synchronized.
     *
    ...
    /**
     * Runs the lazy plugin scan once, and returns only once it completed: readers must not see a
     * half populated map, so the map being non empty cannot be the signal, {@link #scanned} is.
     */
