...
public class ImageWorker {
    ...
    public static final String MOSAIC_CROP_REDUCTION_ENABLED_KEY =
            "org.geotools.image.reduceMosaicCrop";
    ...
    public ImageWorker mosaic(
        ...
        if (MOSAIC_CROP_REDUCTION_ENABLED
                ...
                && images != null
                && images.length == 1
                && images[0] instanceof RenderedOp) {
            RenderedOp crop = (RenderedOp) images[0];
    ...
    /**
     * The nodata range of a single source operation that crops/mosaic by filling with nodata,
     * {@code null} when it does anything more, or when the fill is not a {@link #harmlessFill
     * harmless} one. This "crop" is a mosaic: either a Mosaic operation, which is what {@link
     * org.geotools.coverage.processing.operation.Crop} builds when nodata or a ROI are involved, or
     * a Crop operation, which image processing renders as a mosaic too once it sees the nodata
     * range. A Crop without nodata is instead a pure bounds change, its tiles are the source ones,
     * so there is nothing to fuse and this method returns {@code null} for it.
     */
    private static Range singleSourceNoData(RenderedOp op, double[] background) {
        if (op.getNumSources() != 1 || !(op.getSources().get(0) instanceof RenderedImage))
            return null;
        ...
        if ("Crop".equals(op.getOperationName())) {
            ...
            // fills, hence the count check. The ROI needs no check of its own: the operation
            // reduces
            ...
            return harmlessFill(
                    params.getObjectParameter(5), params.getObjectParameter(6), background);
        }
        if ("Mosaic".equals(op.getOperationName())) {
            ...
            if (!(nodata instanceof Range[]) || ((Range[]) nodata).length != 1) return null;
            return harmlessFill(((Range[]) nodata)[0], params.getObjectParameter(4), background);
        }
        return null;
    }

    /**
     * The nodata range, when the fill that comes with it is one the enclosing mosaic writes anyway:
     * either the fill is nodata itself, so the mosaic matches those pixels and replaces them, or it
     * is the same constant the mosaic fills with. {@code null} when the two differ, dropping the
     * crop would then change pixels.
     *
     * <p>Fill inside the nodata range: crop has {@code NoData = [0,0]} and {@code destNoData =
     * {0}}, mosaic background 255. Before fusion the crop writes 0, then the mosaic matches it
     * against {@code [0,0]} and writes 255. After fusion the mosaic matches the source nodata
     * directly and writes 255, same pixels.
     *
     * <p>Fill equal to the mosaic background: {@code NoData = [0,0]}, {@code destNoData = {255}},
     * background 255. Before fusion the crop writes 255 and the mosaic passes it through, after
     * fusion the mosaic matches the source nodata and fills 255, same pixels again.
     *
     * <p>Rejected instead: {@code NoData = [0,0]}, {@code destNoData = {1}}, background 255. Those
     * pixels are 1 with the crop in place, they would become 255 without it.
     */
    private static Range harmlessFill(Object nodata, Object fill, double[] background) {
        if (!(nodata instanceof Range) || !(fill instanceof double[])) return null;
        Range range = (Range) nodata;
        double[] fillValues = (double[]) fill;
    ...
    /**
     * Bounds pinned by the image layout hint, empty when it does not pin all four. An empty
     * rectangle is contained in nothing, so a missing layout blocks the crop fusion.
     */
    private Rectangle layoutBounds() {
        Object hint = getRenderingHint(JAI.KEY_IMAGE_LAYOUT);
        if (!(hint instanceof ImageLayout)) return new Rectangle();
        ImageLayout layout = (ImageLayout) hint;
