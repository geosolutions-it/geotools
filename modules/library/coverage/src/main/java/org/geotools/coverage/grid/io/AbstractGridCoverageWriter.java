...
public abstract class AbstractGridCoverageWriter implements GridCoverageWriter {
    ...
    /**
     * Computes the {@link ImageOutputStream} to write the geotiff to, based on the type of the
     * destination object. The destination can be a {@link File}, a {@link URL}, an {@link
     * OutputStream} or an {@link ImageOutputStream}.
     *
     * @param image The image that will be written
     * @return The output stream to write the GeoTIFF to.
     * @throws IOException Might be thrown during the construction of the output stream, for
     *     instance if the destination is a file and it cannot be created.
     */
    ...
    protected ImageOutputStream getImageOutputStream(RenderedImage image) throws IOException {
        ...
        if (this.outStream == null)
            throw new IOException(
                    "Could not create an ImageOutputStream for destination: " + destination);
