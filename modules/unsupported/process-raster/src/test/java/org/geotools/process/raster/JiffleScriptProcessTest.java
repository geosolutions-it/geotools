...
public class JiffleScriptProcessTest {
    ...
    private static final String IMAGE_NAME_3 = "multi_band.tiff";
    ...
    public void testSimpleScript() throws Exception {
        final GeoTiffReader reader = new GeoTiffReader(TestData.file(this, IMAGE_NAME_1));
        GridCoverage2D testCoverage = reader.read(null);
        // Script selection
        String script = FileUtils.readFileToString(TestData.file(this, JIFFLE_SCRIPT_PATH));
        // Process calculation
        GridCoverage2D finalCoverage = process.execute(testCoverage, script, null, null);
        // Check the result
        ...
        checkExecution(finalImage, FINAL_VALUES, testCoverage);

        reader.dispose();
        testCoverage.dispose(true);
    ...
    public void testSimpleScript2ImagesCombined() throws Exception {
        final GeoTiffReader reader1 = new GeoTiffReader(TestData.file(this, IMAGE_NAME_1));
        GridCoverage2D testCoverage1 = reader1.read(null);
        final GeoTiffReader reader2 = new GeoTiffReader(TestData.file(this, IMAGE_NAME_2));
        GridCoverage2D testCoverage2 = reader2.read(null);
        ...
        RenderedImage imgComposed = BandMergeDescriptor.create(img1, img2, null);
        ...
        reader1.dispose();
        testCoverage1.dispose(true);
        reader2.dispose();
        testCoverage2.dispose(true);        
        finalCoverage.dispose(true);
        if (finalImage instanceof RenderedOp) {
            ((RenderedOp) finalImage).dispose();
        }
    ...
    public void testSimpleScriptMultiBandedImage() throws Exception {
        final GeoTiffReader reader = new GeoTiffReader(TestData.file(this,
                IMAGE_NAME_3));
        GridCoverage2D testCoverage = reader.read(null);
        ...
        String script = FileUtils.readFileToString(TestData.file(this, JIFFLE_SCRIPT_PATH));
        // Process calculation
        GridCoverage2D finalCoverage = process.execute(testCoverage, script, null, 0);
        // Check the result
        assertNotNull(finalCoverage);
    ...
    public void testWrongBandIndexException() throws Exception {
        ...
        try {
            // Process calculation
            finalCoverage = process.execute(testCoverage, script, null, WRONG_NUM_BANDS);
    ...
    public void testOnAllBands() throws Exception {
        ...
        scripts.add(script);
        GridCoverage2D finalCoverage = process2.execute(testCoverage, scripts);
        // Check the result
        assertNotNull(finalCoverage);
        ...
        int numBandsIn = testCoverage.getRenderedImage().getSampleModel().getNumBands();

        ColorModel cm = testCoverage.getRenderedImage().getColorModel();
        ...
        checkExecution(finalImage, values, testCoverage);
    ...
    public void testOnAllBandsMultiScripts() throws Exception {
        ...
        String script = FileUtils.readFileToString(TestData.file(this, JIFFLE_SCRIPT_PATH));
        // Script2 selection
        String script2 = FileUtils.readFileToString(TestData.file(this, JIFFLE_SCRIPT_2_PATH));
        // Process calculation
        List<String> scripts = new ArrayList<String>(1);
        scripts.add(script);
        scripts.add(script2);
